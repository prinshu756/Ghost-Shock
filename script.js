...
alert("Touch to give them shock")
window.addEventListener('load', () => {
  const shouldPlayMusic = true;

  if (shouldPlayMusic) {
    const sound = new Howl({
      src: ['ghost-dreamer-32184.mp3'],
      autoplay: true,
      volume: 1.0,
    //   onplay: function () {
...
class Game {
	constructor() {
		...
		for (let i = 0; i < 7; i++)
			this.ghosts.push(
				new Ghost(
					this.stage,
					this.stagePadding +
						Math.round(Math.random() * (this.width - pad)),
					this.stagePadding +
						Math.round(Math.random() * (this.height * 0.7 - pad))
				)
			);
		...
		let touchStarts = Rx.Observable.fromEvent(document, 'touchstart').map(
			U.touchEventToCoordinate
		);
		let touchMoves = Rx.Observable.fromEvent(document, 'touchmove').map(
			U.touchEventToCoordinate
		);
...
class Ghost {
	...
	setTarget(position, streamStart) {
		...
		// console.log("hits");
		// should play wali cheeze 
      if (true) {
      const sound = new Howl({
      src: ['657818__the-sacha-rush__electric-shock-4-hit.wav'],
      autoplay: true,
      volume: 1.0,
    //   onplay: function () {
	// 	// plays music
